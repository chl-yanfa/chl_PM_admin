...
function addPay(){
	...
	var voucherId = $(".addpic").eq(0).children("input[type='hidden']").val();
	...
	formData.append("payVoucherId",voucherId);
	$.ajax({
        url : '../../finance/addPay',
        type : 'POST',
        async : false, 
...
function loadData(){
	...
	$.post('../../finance/getPay',{
		...
		if(res.entity!=null){
			var pay=res.entity;
			if(pay.isFrozenTransferMargin ==1){
				$('.load-ignoreVoucherTr').show();
			}
			$('.content-top .title').html("已付款");
			$('.loadPay-DIV').show();
