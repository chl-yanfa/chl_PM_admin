...
if(_71.iconCls){
//TODO 按钮后空格图片注释掉 modify by ggl 20141211 因为分页原因又放开了 
...
var _85=$(_82).addClass("pagination");
$("<div class=\"pagination-info\"></div>").appendTo(_85);
$("<table cellspacing=\"0\" cellpadding=\"0\" border=\"0\"><tr></tr></table>").appendTo(_85);
var tr=_85.find("tr");
...
//XXX 修改分页默认配置文件
$.fn.pagination.defaults={total:1,pageSize:10,pageNumber:1,pageList:[10,20,30,50],loading:false,buttons:null,showPageList:false,showRefresh:true,links:10,layout:["list","first","prev","links","next","last","manual","refresh"],onSelectPage:function(_a7,_a8){
...
$(this).pagination("select",Number(_b0.pageNumber)+Number(1));
}
...
return data;
...
},onMove:function(left,top){/*//XXX 添加禁止将dialog窗口完全移除父窗口方法
	...
	if(winWidth > 200 && left + 200 > winWidth){
		left = winWidth-200;
		isNeedMove = true;
	}
	if(winHeight>100 && top + 100 > winHeight){
		top = winHeight - 100;
		isNeedMove = true;
	}
	...
	if(isNeedMove){
		if($(this).attr("id")!='')
			$("#"+$(this).attr("id")).panel("move",{left:left,top:top});
	}
*/},onMaximize:function(){
},onRestore:function(){
},onMinimize:function(){
...
opts.top=(_24c.height()-_24b)/2+_24c.scrollTop();
	...
	if(window.top==window.self){
		...
		var topTemp = $(window.parent.document).scrollTop()-70;
		topTemp = topTemp<100?100:topTemp;
		var documentHeight = $(document).height();
		if(_24b+topTemp>documentHeight){//XXX 修正弹出框下方会出屏的问题
			topTemp = documentHeight - _24b;
		}
		opts.top=topTemp;
	}
}
...
var _266=$("<div class=\"dialog-toolbar\"><table cellspacing=\"0\" cellpadding=\"0\" width='100%'><tr><td></td></tr></table></div>").prependTo(_264);
var tr=_266.find("td");
for(var i=0;i<opts.toolbar.length;i++){
var btn=opts.toolbar[i];
//XXX 工具栏支持字符串添加
if(btn=="-"){
$("<div class=\"dialog-tool-separator\"></div>").appendTo(tr);
}else if(btn=="|"){
$("<br/><br/>").appendTo(tr);
}else if(typeof btn=="string"){
$("<div style='line-height:26px'>&nbsp;"+btn+"&nbsp;</div>").appendTo(tr);
}else{
//XXX 修改工具栏按钮靠右对齐
var td;
var btnBorderStyle="style='";
if(btn){
if(btn.align=='right'){
	btnBorderStyle+="border:none;background:none";
	td=$("<div style='line-height:26px;float:right;width:auto;word-break:keep-all'></div>").appendTo(tr);
}else{
	td=$("<div style='line-height:26px;float:left;width:auto;word-break:keep-all'></div>").appendTo(tr);
}
if(btn.style){
	btnBorderStyle+=";"+btn.style;
}
btnBorderStyle+="'";
var tool=$("<a href=\"javascript:void(0)\" "+btnBorderStyle+"></a>").appendTo(td);
...
//XXX 移除alert的关闭按钮 add by 刘民
win.parent().find("div.panel-tool").remove();
return win;
...
//XXX 去掉页面最前端“|” add by ggl
var shugang = tabs.find("li").length==0?"&nbsp;":"|";
opts.tab=$("<li></li>").appendTo(tabs);
opts.tab.append("<samp class='line'>"+shugang+"</samp><a href=\"javascript:void(0)\" class=\"tabs-inner\"><span class=\"tabs-title\"></span></a>");
...
//XXX 调整搜索录入域边框为26px，span样式为24px modify by ggl
//sb._outerWidth(opts.width)._outerHeight(opts.height);
sb._outerWidth(opts.width)._outerHeight(26);
_402._outerWidth(sb.width()-menu._outerWidth()-_401._outerWidth());
//XXX 调整搜索录入域前加5px modify by ggl
_402.css({height:sb.height()+"px",lineHeight:sb.height()+"px",paddingLeft:"5px"});
...
//$(_4ed).focus();  //XXX 注释掉是因为easyui默认认为页面只会有一个datatimebox控件，获取焦点时会导致datagrid的tab选项卡丢失
};
...
//XXX 微调datagrid头部宽度，解决右侧边框不显示的问题
_51d.children("div.datagrid-header,div.datagrid-body,div.datagrid-footer").width(_51d.width()-2);
_51e.children("div.datagrid-header,div.datagrid-body,div.datagrid-footer").width(_51e.width()-2);
var hh;
_51f.css("height","");
_520.css("height","");
_521.css("height","");
...
if (opts.fitStyle == true) {
	//XXX 调整datagrid高度以适应显示区域高度  解决没有使用tab时页面下方有一截空白的问题
	var toolbarHeight = wrap.find("div.datagrid-toolbar").find("table").height();
	var footbarHeight = wrap.find("div.datagrid-pager").height();
	var tabHeight = 36;
	if (opts.tabUse == false) {
		tabHeight = -10;//XXX 根据实际页面情况调整来的
	}
	var _fitHeight = _524 - _525 - tabHeight - toolbarHeight - footbarHeight;
	_51d.add(_51e).children("div.datagrid-body").css({marginTop:_525,height:(_fitHeight)});
}
...
var tb=$("<div class=\"datagrid-toolbar\"><table cellspacing=\"0\" cellpadding=\"0\" width='100%'><tr><td></td></tr></table></div>").prependTo(_548);
var tr=tb.find("td");
for(var i=0;i<opts.toolbar.length;i++){
var btn=opts.toolbar[i];
//XXX 添加支持字符串在工具栏中
if(btn=="-"){
$("<div class=\"datagrid-btn-separator\" style='float:left'></div>").appendTo(tr);
}else if(btn=="|"){
$("<br/><br/>").appendTo(tr);
}else if(typeof btn == "string"){
$("<div style='line-height:26px;float:left;width:auto'>&nbsp;"+btn+"&nbsp;</div>").appendTo(tr);	
}else{
var td;
var btnBorderStyle="style='";
//XXX 添加靠右对其属性
...
if(btn.align=='right'){
	btnBorderStyle+="border:none;background:none";
	td=$("<div style='line-height:26px;float:right;width:auto;margin-right:10px;word-break:keep-all'></div>").appendTo(tr);
}else{
	td=$("<div style='line-height:26px;float:left;width:auto;word-break:keep-all'></div>").appendTo(tr);
}
if(btn.style){
...
//XXX 添加自定义列表头部内容 addby 刘民
if (col.sortable && typeof(col.defaultOrder) != 'undefined') {
	$("div", td).addClass("datagrid-sort-" + col.defaultOrder);
}
if(col.headFormatter){
	$("span",td).html(col.headFormatter());
}else{
...
/**
 * 默认有多个排序字段时,以点击字段为判断首位
 * xuejun1.li
 * 2015.04.13
 */
if (opts.multiSort) {
	pos = _502(_571, _573);
	var _tSortName = _571[0];
	var _tSortOrder = _572[0];

	_571[0] = _571[pos];
	_572[0] = _572[pos];

	_571[pos] = _tSortName;
	_572[pos] = _tSortOrder;
}
...
if(top<0){
//	var _scrollTop1 = _5c5.scrollTop() + top - Math.abs(_5c5.outerHeight() - _5c5.scrollTop());
	if (_5c2 > 0) {
...
//XXX 修改选中样式
var selectTr_526 = opts.finder.getTr(_5cc,_5cd).addClass("datagrid-row-selected");
if(opts.rowTitleFormatter){
	selectTr_526.prev("tr.rowTitleStyle").addClass("rowTitleSeleted");
}else{
	selectTr_526.addClass("datagrid-row-selected1");
}
...
//XXX 添加取消选中样式方法
var selectTr_526=opts.finder.getTr(_5e1,"","selected").removeClass("datagrid-row-selected");
if(opts.rowTitleFormatter){
	selectTr_526.prev("tr.rowTitleStyle").removeClass("rowTitleSeleted");
}else{
	selectTr_526.removeClass("datagrid-row-selected1");
}
if(opts.idField){
...
if(typeof opts.autoLoad != undefined && opts.autoLoad == false){  //XXX 如果设置了不自动加载，就让datagrid不自动提交
	return;
}
_577(this);
});
...
if(!opts.nowrap){
_6e9+="white-space:normal;height:auto;word-break:break-all";
}else{
...
//XXX 调整日期录入域样式高度 modify by ggl
_842._outerWidth(opts.width)._outerHeight(26);
...
var selectedItem = _8b5.find("div.combobox-item-selected");
selectedItem.removeClass("combobox-item-selected");
//XXX 修改默认combox未选中方法
selectedItem.find("input:first").prop("checked",false);
var vv=[],ss=[];
for(var i=0;i<_8b3.length;i++){
var v=_8b3[i];
var s=v;
//XXX 修改默认combox选中样式
opts.finder.getEl(_8b2,v).addClass("combobox-item-selected").find("input:first").prop("checked",true);
...
//XXX 修改combox默认样式
if(opts.multiple){
	dd.push("<input type='checkbox' style='margin-bottom: 1px;vertical-align: bottom'/>");
}else{
	dd.push("<input type='radio' name='"+(_8b9.itemIdPrefix)+"' style='margin-bottom: 1px;vertical-align: bottom'/>");
}
...
//XXX 修改默认combox样式
_8d4.find("div.combobox-item-selected").removeClass("combobox-item-selected").find("input:first").prop("checked",false);
});
},reset:function(jq){
return jq.each(function(){
