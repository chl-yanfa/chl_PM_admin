	...
	$("#list").datagrid({
        ...
        columns : [[
                ...
                {title : '操作', field : 'id', width : 120, align : 'center',formatter:function(value,row,index){
					...
					var audit= "";
					if(row.deferStateFlag == 1){
						audit="audit('"+auctionId+"','"+auctionSetId+"',0)";												
					}else if(row.adjustPriceStateFlag == 1){
						audit="audit('"+auctionId+"','"+auctionSetId+"',1)";
					}else if(row.backCarStateFlag == 1){
						audit="audit('"+auctionId+"','"+auctionSetId+"',2)";
					}
					var optionStr = '';
					var styleStr='href="javascript:void(0);" style="font-size: 14px;color: #4EA2FF;"';
					if(row.deferStateFlag == 1 || row.adjustPriceStateFlag == 1 ||row.backCarStateFlag == 1){
						optionStr += '<a '+styleStr+' onclick="'+audit+'">审批</a><br>';						
					}
					return optionStr;
               }}
              ]],
...
function reLoadData(page){
	...
	$("#list").datagrid(loadType, {
		...
		afAuditState: currentStatus
    });
}

//初始化Tab页
function initTabs(){
	$.post(URLMODULE.getAbnormalTab+'?_' + Math.random(),function(data) {
...
function getTabCountNumber(data,key){
	...
	for(var i=0;i<data.length;i++){
		if(data[i].af_audit_state == key){//data数据与indexToStatus关联
...
//拍品状态
var TabsStatus = {0:'全部',1:'待审核',2:'审核通过',3:'已驳回'};
var stateToIndex = {'全部':-1,'待审核':0,'审核通过':1,'已驳回':2};

//更新Tab页
function updateTab(type){
	$.post(URLMODULE.getAbnormalTab+'?_' + Math.random(),{
		auctionType:$('#tb-auctionType').combobox('getValue'),
		dealDateStart: $('#tb-dealDateStart').datetimebox("getValue"),
		dealDateEnd: $('#tb-dealDateEnd').datetimebox("getValue"),
		keywords: $('#searchText').searchbox('getValue').trim()
...
function audit(auctionId,auctionSetId,type){
	window.location.href="../../html/aftersale/audit.html?auctionId="+auctionId+"&auctionSetId="+auctionSetId+"&type="+type;
}