window.onload=jQuery(function(){
    ...
    var uploader = WebUploader.create({
        ...
        accept:{
            title: 'Images',
            extensions: 'jpg,bmp,png',//'gif,jpg,jpeg,bmp,png'
            mimeTypes: 'images/*'
        },
        resize: false,
    ...
    if(imglist3!=null&&imglist3.length>0){
		for(var i=0;i<imglist3.length;i++){
			var file = new Object();
			file.path=imglist3[i];
			file.src='server';
			file.name=imglist3[i];
			file.id="WU_FILE_-"+(i+1);
			fileQueue(file);
		}
		imglist3=[];
    }
...
//            text = '共' + fileCount +'张('+WebUploader.formatSize(fileSize)+')，已上传'+stats.successNum+'张';
    ...
    $upload.on('click', function(){
        ...
        var obj = new Object();
        obj.stockId = $('#addWarehouse input[name="id"]').val();
