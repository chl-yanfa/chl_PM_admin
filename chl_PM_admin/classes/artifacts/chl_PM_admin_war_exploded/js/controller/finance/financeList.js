...
//初始化Tab页
function initTabs(){
	$.post(URLMODULE.getFinanceTab+'?_' + Math.random(),function(data) {
...
//拍品状态
var TabsStatus = {0:'全部',1:'待付款',2:'已付款',3:'延期未付款'};
var stateToIndex = {'全部':-9,'待付款':0,'已付款':1,'延期未付款':2};

//更新Tab页
function updateTab(type){
	$.post(URLMODULE.getFinanceTab+'?_' + Math.random(),{
		auctionType:$('#tb-auctionType').combobox('getValue'),
		paimaiDateStart: $('#tb-paimaiDateStart').datetimebox("getValue"),
		paimaiDateEnd: $('#tb-paimaiDateEnd').datetimebox("getValue"),
		keywords: $('#searchText').searchbox('getValue').trim()
...
function payTrace(auctionId,auctionSetId){
	window.location.href="../../html/finance/financePayTrack.html?id="+auctionId+"&auctionSetId="+auctionSetId;
}